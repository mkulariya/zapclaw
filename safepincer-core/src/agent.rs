...
use crate::llm::{
    ChatMessage, LlmClient, ToolCall, ToolDefinition, StreamChunk,
};
...
/// Events emitted during agent execution (streaming).
#[derive(Debug, Clone)]
pub enum AgentEvent {
    /// Reasoning token (inside <think>...</think>)
    Think(String),
    /// Final output token (user-visible)
    Content(String),
    /// Tool execution started
    ToolStart { name: String, args: String },
    /// Tool output received
    ToolOutput { name: String, output: String },
    /// Step completion (or intermediate info)
    Step { current: usize, max: usize },
    /// Fatal error
    Error(String),
    /// Task completed
    Done,
...
/// Tool trait — implemented by all SafePincer tools.
///
...
/// The SafePincer Agent — core observe-plan-act-reflect loop.
///
/// This is the heart of SafePincer. It processes tasks by:
...
impl Agent {
    ...
    fn build_system_prompt(&self) -> String {
        ...
        // Identity
        sections.push("You are SafePincer 🦞, a secure and efficient personal AI assistant.".to_string());
