...
impl MemoryDb {
    ...
    pub fn new(workspace: &Path) -> Result<Self> {
        ...
        let fts_available = ensure_schema(&db);
        if !fts_available {
    ...
    pub fn in_memory() -> Result<Self> {
        let tmp = std::env::temp_dir().join(format!("safepincer_mem_{}", uuid::Uuid::new_v4()));
    ...
    pub fn sync(&self, model: &str) -> Result<usize> {
        ...
        let stale_paths: Vec<String> = {
            let mut stmt = self.db.prepare("SELECT path FROM files WHERE source = ?")?;
    ...
    pub fn index_status(&self) -> Result<IndexStatus> {
        let files: i64 = self.db.query_row(
            "SELECT COUNT(*) FROM files", [], |r| r.get(0),
        ).unwrap_or(0);
        let chunks: i64 = self.db.query_row(
            "SELECT COUNT(*) FROM chunks", [], |r| r.get(0),
        ).unwrap_or(0);
        let embedded: i64 = self.db.query_row(
