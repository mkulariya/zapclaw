...
use clap::Parser;
use safepincer_core::agent::{Agent, ToolRegistry};
use safepincer_core::config::{Config, LlmMode};
use safepincer_core::confiner::Confiner;
use safepincer_core::llm::OpenAiCompatibleClient;
use safepincer_core::memory::MemoryDb;
use safepincer_tools::browser_tool::BrowserTool;
use safepincer_tools::cron_tool::{check_due_jobs, CronTool};
use safepincer_tools::edit_tool::EditTool;
use safepincer_tools::exec_tool::ExecTool;
use safepincer_tools::file_tool::FileTool;
use safepincer_tools::find_tool::FindTool;
use safepincer_tools::grep_tool::GrepTool;
use safepincer_tools::image_tool::ImageTool;
use safepincer_tools::math_tool::MathTool;
use safepincer_tools::memory_tool::{MemoryGetTool, MemorySearchTool};
use safepincer_tools::patch_tool::PatchTool;
use safepincer_tools::process_tool::ProcessTool;
use safepincer_tools::session_tool::SessionTool;
use safepincer_tools::web_search_tool::WebSearchTool;
use std::sync::Arc;
use std::path::PathBuf;

/// SafePincer 🦞 — Secure, lightweight AI agent
#[derive(Parser, Debug)]
#[command(name = "safepincer", version, about = "SafePincer 🦞 — Secure, lightweight AI agent")]
struct Cli {
    /// Workspace directory path
    #[arg(short, long, default_value = "./safepincer_workspace")]
    workspace: String,
    ...
    /// API key (prefer SAFEPINCER_API_KEY env var for security)
    #[arg(long, env = "SAFEPINCER_API_KEY")]
    api_key: Option<String>,

    /// Web search API key (for Brave Search)
    #[arg(long, env = "SAFEPINCER_SEARCH_API_KEY")]
...
async fn self_update() -> Result<()> {
    ...
    let client = reqwest::Client::builder()
        .user_agent("SafePincer-Updater")
        .build()?;

    let resp = client
        .get("https://api.github.com/repos/safepincer/safepincer/releases/latest")
        .send()
        .await
    ...
    if output.status.success() {
        println!("✅ Updated successfully! Restart SafePincer to use the new version.");
    } else {
...
async fn main() -> Result<()> {
    ...
    println!("🦞 SafePincer v{}", env!("CARGO_PKG_VERSION"));
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    ...
    let llm: Arc<dyn safepincer_core::llm::LlmClient> = Arc::new(
    ...
    if let Some(task) = cli.task {
        let response = agent.run(&session_id, &task).await?;
...
🦞 SafePincer Help
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Just type your request and SafePincer will help!
  ...
  Skills:
    ...
    SafePincer will auto-discover and use them.
