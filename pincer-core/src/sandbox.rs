...
//!
//! Pincer runs inside a bubblewrap (bwrap) sandbox by default.
//! This module provides:
//! - Detection of sandbox state via `PINCER_SANDBOXED` env var
...
///
/// If already sandboxed (`PINCER_SANDBOXED=1`), returns `SandboxState::Active`.
...
pub fn ensure_sandboxed(workspace: &Path, no_network: bool) -> Result<SandboxState> {
    ...
    cmd.args(["--hostname", "pincer-sandbox"]);
    ...
    cmd.args(["--setenv", "PINCER_SANDBOXED", "1"]);            // Signal to child
...
mod tests {
    ...
    fn test_is_sandboxed_false_by_default() {
        // In test environment, PINCER_SANDBOXED should not be set
        // (unless tests are run inside the sandbox)
        if std::env::var("PINCER_SANDBOXED").is_err() {
            assert!(!is_sandboxed());
        }
    }
