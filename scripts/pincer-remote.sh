#!/usr/bin/env bash
#
# pincer-remote — Remote client for Pincer agent
#
# Sends tasks to a Pincer instance over SSH tunnel + JSON-RPC.
#
# Requirements: curl, jq, ssh, base64
#
# Environment variables:
#   PINCER_REMOTE_HOST   SSH host (user@server) — required for tunnel
#   PINCER_REMOTE_KEY    API key for authentication — required
#   PINCER_REMOTE_PORT   RPC port (default: 9876)
#
# Usage:
#   pincer-remote "What is 2+2?"                  # One-shot task
#   pincer-remote -i                               # Interactive mode
#   pincer-remote --health                         # Health check
#   pincer-remote --upload local.txt [remote.txt]  # Upload file
#   pincer-remote --download remote.txt [local.txt] # Download file
#   pincer-remote --list [pattern]                 # List workspace files

set -euo pipefail

PORT="${PINCER_REMOTE_PORT:-9876}"
HOST="${PINCER_REMOTE_HOST:-}"
KEY="${PINCER_REMOTE_KEY:-}"
...
check_key() {
    [ -n "$KEY" ] || die "PINCER_REMOTE_KEY not set. Set it to your Pincer inbound API key."
}
...
rpc_call() {
    ...
    response=$(curl -s --max-time "$timeout" "$BASE_URL" \
        ...
        -d "$body" 2>&1) || die "Connection failed. Is Pincer running with --enable-inbound?"

    # Check for HTTP-level errors
    if echo "$response" | grep -q "Invalid or missing API key"; then
        die "Authentication failed. Check PINCER_REMOTE_KEY."
    fi
...
ensure_tunnel() {
    # If we can already connect, tunnel exists or Pincer is local
    if check_connection; then
        return 0
    fi

    # Need SSH host to create tunnel
    [ -n "$HOST" ] || die "Cannot connect to localhost:${PORT} and PINCER_REMOTE_HOST not set.\nEither run Pincer locally or set PINCER_REMOTE_HOST for SSH tunnel."
    ...
    while ! check_connection; do
        ...
        if [ "$attempts" -ge 10 ]; then
            die "SSH tunnel opened but cannot reach Pincer on port $PORT."
        fi
        sleep 0.5
    done
...
cmd_interactive() {
    echo "Pincer Remote (interactive mode)"
...
case "${1:-}" in
    -h|--help)
        echo "Usage: pincer-remote [OPTIONS] [TASK]"
        echo ""
        echo "Options:"
        ...
        echo ""
        echo "Environment:"
        echo "  PINCER_REMOTE_HOST   SSH host (user@server)"
        echo "  PINCER_REMOTE_KEY    API key (required)"
        echo "  PINCER_REMOTE_PORT   Port (default: 9876)"
        ;;
    --health)
        check_key
        ensure_tunnel
